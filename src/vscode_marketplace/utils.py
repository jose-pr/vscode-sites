...
## https://stackoverflow.com/questions/35640871/is-is-possible-to-clean-a-verbose-python-regex-before-printing-it
import re

_VERBOSE_REGEX = re.compile(r"""(?#!py/mx decomment Rev:20160225_1800)
...
def regex_as_string(regex:re.Pattern):
    if re.VERBOSE in re.RegexFlag(regex.flags):
        def detidy_cb(m):
            if m.group(2): return m.group(2)
            if m.group(3): return m.group(3)
            return ""
        return re.sub(_VERBOSE_REGEX, detidy_cb, regex.pattern)
    else:
